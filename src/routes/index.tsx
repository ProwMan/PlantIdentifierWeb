...
function Index() {
  ...
  async function handleAuth() {
    ...
    try {
      const { uid, displayName } = await login({ data: { fullName, className } });
      const email = `${uid}@acsi.efg`;
      const password = `acsi-${uid.slice(0, 10)}`;
      ...
      // Update user info in Firestore from the client
      await setDoc(
        doc(db, "users", uid),
        {
  ...
  async function startCamera() {
    ...
    if (!navigator.geolocation) {
      alert("Location access is required to identify plants.");
      return;
    }
  ...
  async function confirmDiscovery(suggestion: PlantSuggestion) {
    ...
    if (!foundBefore) {
      const todayNewDiscoveries = discoveries.filter(
        (d) => d.userId === user.uid && d.points === 10 && new Date(d.foundAt) >= startOfDay
      ).length;
      
      if (todayNewDiscoveries >= 10) {
        setError("You have reached the maximum limit of 10 new discoveries for today. Come back tomorrow!");
        setPhase("menu");
        return;
      }
    } else {
      const todayRediscoveries = discoveries.filter(
        (d) => d.userId === user.uid && d.points === 1 && new Date(d.foundAt) >= startOfDay
      ).length;
      
      if (todayRediscoveries >= 10) {
        setError("You have reached the maximum limit of 10 rediscoveries for today. Come back tomorrow!");
        setPhase("menu");
        return;
      }
    }
